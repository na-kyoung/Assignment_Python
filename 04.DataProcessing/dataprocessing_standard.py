# 데이터 전처리 - 정규화 표준화(Standardization)
# 0. 데이터 및 그래프 출력
# 1. 전체 데이터 수 확인
# 2. 결측치 확인
# 3. Null값 확인
...
# 그래프 출력
cols = len(df_standard.columns) - 1
# rows = len(df)

df_standard['Axis [nm]'] = df_standard['Axis [nm]'].replace('[\$,]', '', regex=True).astype(float)
x = df_standard['Axis [nm]']

plt.figure(figsize=(15, 8))
for i in range(1,cols+1):
    colName = f'ROI {i} []'
    df_standard[colName] = df_standard[colName].replace('[\$,]', '', regex=True).astype(float)
    y = df_standard[colName]
